 ...
 *   $donnees['depenses']        → Collection BonCommandeRegie
 *   $donnees['lignesApurement'] → Collection de strings
...
$lignesApurement = $donnees['lignesApurement'];
$numeroTranche   = $donnees['numeroTranche'] ?? '';
$dateDebut       = $donnees['dateDebut'];
$dateFin         = $donnees['dateFin'];
$totaux          = $donnees['totaux'];

// ✅ Entête via EtatConfig — même pattern que bon-commande-simple
...
// Paysage : largeur utile ≈ 270mm → plus de chars/ligne
$charsFournisseurParLigne = 26;
$charsNumeroParLigne      = 55;

$calcPoids = function ($d) use ($charsFournisseurParLigne, $charsNumeroParLigne) {
    $pF = max(1, (int) ceil(mb_strlen($d->fournisseur?->raison_sociale ?? '') / $charsFournisseurParLigne));
    $pN = max(1, (int) ceil(mb_strlen($d->numero ?? '') / $charsNumeroParLigne));
    return max($pF, $pN);
};
...
<head>
<meta charset="UTF-8">
<title>Compte d'Emploi — {{ $regie->numero }}</title>
<style>
    @page {
    ...
    .titre-doc {
        ...
        background:#eee; border:1px solid #000; padding:7px 4px; margin-bottom:3px;
    }
    .sous-titre  { text-align:center; font-weight:bold; font-size:9.5pt; margin-bottom:8px; }

    .liste-apurement { font-size:8.5pt; margin-bottom:8px; }
    .liste-apurement table { width:100%; border-collapse:collapse; }
    .liste-apurement td    { border:none; padding:1px 0; vertical-align:top; }
    .liste-apurement .num  { width:22px; font-weight:bold; }

    table.depenses { width:100%; border-collapse:collapse; font-size:8pt; table-layout:fixed; }
    table.depenses th {
        border:1px solid #000; background:#e6e6e6; padding:5px 3px;
        text-align:center; font-weight:bold; font-size:8pt;
    }
    table.depenses td {
        border:1px solid #000; padding:4px 3px; font-size:8pt;
...
<div class="pdf-footer">
    <table>
        <tr>
            ...
            <td style="width:34%; text-align:right;"><strong>Compte d'Emploi</strong></td>
        </tr>
    </table>
    <div class="pag">
...
@if ($i === 0)
    ...
    <div class="titre-doc">
        APUREMENT DE {{ strtoupper($numeroTranche ?: 'L\'ENCAISSE') }}
    ...
    <div class="sous-titre">COMPTE D'EMPLOI</div>

    @if ($lignesApurement->count() > 0)
    <div class="liste-apurement">
        <table>
            @foreach ($lignesApurement as $idx => $ligne)
            <tr>
    ...
    <div style="font-weight:bold; font-size:9pt; margin:6px 0 4px;">Tableau 1 :</div>
@else
...
<table class="depenses">
    <colgroup>
        ...
        <col class="col-ttc"><col class="col-tva"><col class="col-ir"><col class="col-net">
    </colgroup>
    <thead>
        <tr>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="money">{{ number_format($d->montant_tva, 0, ',', ' ') }}</td>
            <td class="money">{{ number_format($d->montant_ir,  0, ',', ' ') }}</td>
            <td class="money">{{ number_format($d->net_a_payer, 0, ',', ' ') }}</td>
        </tr>
        @empty
        <tr><td colspan="7" style="text-align:center; padding:10px;">Aucune dépense sur la période sélectionnée</td></tr>
        @endforelse
    </tbody>
</table>
...
@if ($loop->last)
    <table class="depenses" style="margin-top:0;">
        <tr class="total-row">
            ...
            <td class="money">{{ number_format($totaux['net'], 0, ',', ' ') }}</td>
        </tr>
    </table>

    <div style="margin-top:12px; font-size:8.5pt; page-break-inside:avoid;">
        Arrête le présent compte statistique à un montant de IR reversé
        <strong>{{ number_format($totaux['ir'], 0, ',', ' ') }}</strong> FCFA
        ({{ ucfirst(\App\Helpers\NombreEnLettres::montantCFA($totaux['ir'] ?? 0)) }}).
    </div>
    <div style="margin-top:30px; text-align:right; font-size:9pt; page-break-inside:avoid;">
        Le Régisseur
    </div>
@else

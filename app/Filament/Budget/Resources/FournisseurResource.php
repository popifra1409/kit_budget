...
use App\Filament\Budget\Resources\FournisseurResource\Pages;
...
class FournisseurResource extends Resource
{
    ...
    {
        return auth()->user()?->can('view_any_fournisseur') ?? false;
    }

    public static function canCreate(): bool
    {
        return auth()->user()?->can('create_fournisseur') ?? false;
    }

    public static function canEdit($record): bool
    {
        return auth()->user()?->can('update_fournisseur') ?? false;
    }

    public static function canDelete($record): bool
    {
        return auth()->user()?->can('delete_fournisseur') ?? false;
    }

    public static function canView($record): bool
    {
        return auth()->user()?->can('view_fournisseur') ?? false;
    }

    public static function canActiver($record): bool
    {
        return auth()->user()?->can('toggle_fournisseur') ?? false;
    }
        ...
        return $form
            ->schema([
                Forms\Components\Section::make('Identification')
                    ->schema([
                        Forms\Components\TextInput::make('code')
                            ...
                            ->maxLength(50)->placeholder('Ex: FRS-001'),

                        Forms\Components\TextInput::make('raison_sociale')
                            ->label('Raison sociale')->required()->maxLength(255)->columnSpanFull(),

                        Forms\Components\TextInput::make('sigle')
                            ->label('Sigle / Nom commercial')->maxLength(255)->placeholder('Ex: CAMTEL'),

                        Forms\Components\TextInput::make('rccm')
                            ->label('N° RCCM')->maxLength(255)->placeholder('Ex: RC/DLA/2020/B/1234'),

                        Forms\Components\TextInput::make('nif')
                            ->label('NIF')->maxLength(255)->placeholder('Ex: M012345678901Z'),

                        Forms\Components\TextInput::make('forme_juridique')
                            ->label('Forme juridique')->maxLength(255)->placeholder('Ex: SARL, SA, EI'),
                            ...
                            ->schema([
                                Forms\Components\Select::make('regime_fiscal_id')
                                    ...
                                    ->helperText(function ($get) {
                                        ...
                                        if ($regimeId) {
                                            ...
                                            if ($regime) return "Taux IR par défaut : {$regime->taux_ir_defaut}%";
                                        }
                                ...
                                Forms\Components\Placeholder::make('info_regime')
                                    ...
                                    ->content(function ($get) {
                                        ...
                                        if ($regimeId) {
                                            $regime = RegimeFiscal::find($regimeId);
                                            if ($regime) return $regime->description;
                                        }
                                        return '';
                                    })
                ...
                Forms\Components\Section::make('Coordonnées')
                    ->schema([
                        Forms\Components\TextInput::make('adresse')
                            ->label('Adresse')->maxLength(255)->columnSpanFull(),
                        Forms\Components\TextInput::make('ville')
                            ->label('Ville')->maxLength(255)->default('Douala'),
                        Forms\Components\TextInput::make('pays')
                            ->label('Pays')->maxLength(255)->default('Cameroun'),
                        Forms\Components\TextInput::make('telephone')
                            ->label('Téléphone')->tel()->maxLength(255),
                        Forms\Components\TextInput::make('email')
                            ->label('Email')->email()->maxLength(255),
                        Forms\Components\TextInput::make('site_web')
                            ->label('Site web')->url()->maxLength(255)->placeholder('https://'),
                    ])
                    ->columns(3),

                Forms\Components\Section::make('Contact Principal')
                    ->schema([
                        Forms\Components\TextInput::make('contact_nom')->label('Nom du contact')->maxLength(255),
                        Forms\Components\TextInput::make('contact_fonction')->label('Fonction')->maxLength(255),
                        Forms\Components\TextInput::make('contact_telephone')->label('Téléphone')->tel()->maxLength(255),
                ...
                Forms\Components\Section::make('Informations Bancaires')
                    ->schema([
                        Forms\Components\TextInput::make('banque')->label('Banque')->maxLength(255),
                        Forms\Components\TextInput::make('iban')
                            ->label('IBAN')->maxLength(255)->placeholder('Ex: CM21 1234 5678 9012 3456 7890 1234'),
                        Forms\Components\TextInput::make('code_swift')
                            ->label('Code SWIFT/BIC')->maxLength(255)->placeholder('Ex: BICMCMCX'),
                        Forms\Components\TextInput::make('numero_compte')->label('Numéro de compte')->maxLength(255),
                    ])
                ...
                Forms\Components\Section::make('Statut')
                    ->schema([
                        ...
                        Forms\Components\Toggle::make('blackliste')->label('Blacklisté')->default(false)->reactive(),
                        Forms\Components\Textarea::make('motif_blacklist')
                            ->label('Motif de blacklist')->rows(3)
                            ->visible(fn(callable $get) => $get('blackliste'))->columnSpanFull(),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('raison_sociale')
                    ->label('Raison sociale')->searchable()->sortable()->wrap()->limit(40),

                Tables\Columns\TextColumn::make('sigle')
                    ->label('Sigle')->searchable()->placeholder('-')->toggleable(),
                ...
                Tables\Columns\TextColumn::make('telephone')
                    ->label('Téléphone')->searchable()->placeholder('-')->toggleable(),
                ...
                Tables\Columns\TextColumn::make('ville')
                    ->label('Ville')->searchable()->toggleable(isToggledHiddenByDefault: true),

                Tables\Columns\TextColumn::make('bons_commande_count')
                    ->label('BC')->counts('bonsCommande')->badge()->color('success'),

                // ✅ Nombre de dossiers
                Tables\Columns\TextColumn::make('dossiers_count')
                    ->label('Dossiers')->counts('dossiers')->badge()->color('info'),

                Tables\Columns\IconColumn::make('actif')
                    ->label('Actif')->boolean(),

                Tables\Columns\IconColumn::make('blackliste')
                    ->label('Blacklisté')->boolean()
            ...
            ->filters([
                ...
                Tables\Filters\TernaryFilter::make('blackliste')
                    ->label('Blacklisté')->placeholder('Tous')
                    ->trueLabel('Blacklistés')->falseLabel('Non blacklistés'),
            ])
                ...
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\Action::make('blacklister')
                        ...
                        ->form([
                            Forms\Components\Textarea::make('motif')
                                ->label('Motif de blacklist')->required()->rows(3),
                        ])
                        ->action(function ($record, array $data) {
                            $record->blacklister($data['motif']);
                            Notification::make()->title('Fournisseur blacklisté')->success()->send();
                        }),
                    ...
                    Tables\Actions\Action::make('rehabiliter')
                        ...
                        ->action(function ($record) {
                            $record->rehabiliter();
                            Notification::make()->title('Fournisseur réhabilité')->success()->send();
                        }),

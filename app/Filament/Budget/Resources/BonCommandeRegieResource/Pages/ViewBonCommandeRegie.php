    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('changer_mode_arrondi')
                ...
                ->action(function () {
                    try {
                        ...
                        $this->redirect(
                            BonCommandeRegieResource::getUrl('view', ['record' => $this->record->id])
                        );
                    } catch (\Exception $e) {
                        Notification::make()
                            ->danger()->title('❌ Erreur')
            ...
            Actions\Action::make('valider')
                ...
                ->modalHeading(fn() => 'Aperçu — BC N° ' . $this->record->numero)
                ->modalDescription(new \Illuminate\Support\HtmlString(
                    '<div class="text-sm text-amber-700 bg-amber-50 border border-amber-300 rounded p-2 mb-2">'
                    . '⚠️ Vérifiez toutes les informations avant de valider. '
                    . 'Un BC validé ne peut être modifié qu\'après annulation.'
                    . '</div>'
                ))
                ->modalContent(fn() => view('filament.modals.apercu-bon-commande', [
                    'bc' => $this->record->load([

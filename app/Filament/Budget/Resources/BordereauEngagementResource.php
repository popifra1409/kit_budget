...
use App\Filament\Budget\Resources\BordereauEngagementResource\Pages;
use App\Filament\Budget\Resources\BordereauEngagementResource\RelationManagers;
...
class BordereauEngagementResource extends Resource
{
    ...
    {
        $user = auth()->user();
        if (!$user?->can('update_bordereau_engagement')) return false;
        if (!$record->estModifiable()) return false;
        if ($user->can('override_bordereau_engagement')) return true;
                ...
                Forms\Components\Section::make('Informations principales')
                    ->schema([
                        ...
                        Forms\Components\DatePicker::make('date_emission')
                            ->label('Date d\'émission')->default(now())->required(),
                ...
                Forms\Components\Section::make('Objet')
                    ->schema([
                        Forms\Components\Textarea::make('objet')
                            ->label('Objet du bordereau')->required()->rows(3)
                            ->placeholder('Ex: Transmission engagements décembre 2026')->columnSpanFull(),
                        ...
                        Forms\Components\Select::make('engagements')
                            ...
                            ->relationship(
                                ...
                                modifyQueryUsing: fn($query) => $query
                                    ...
                                    ->whereDoesntHave('bordereaux', fn($q) => $q->where('statut', 'valide'))
                ...
                Forms\Components\Section::make('Informations automatiques')
                    ->schema([
                        ...
                        Forms\Components\Placeholder::make('montant_total')
                            ...
                            ->content(
                                fn($record) => $record
                                    ? number_format($record->montant_total, 0, ',', ' ') . ' FCFA'
                                    : '0 FCFA'
                            ),
                    ])
                    ->columns(3)
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('numero')
                    ->label('N° Bordereau')->searchable()->sortable()->weight('bold')->copyable(),

                Tables\Columns\TextColumn::make('budget.code')
                    ->label('Budget')->searchable()->badge()->color('info'),
                ...
                Tables\Columns\TextColumn::make('emetteur.name')
                    ->label('Émis par')->searchable()->toggleable(),

                Tables\Columns\TextColumn::make('instance_destinataire')
                    ->label('Destinataire')->searchable()->limit(30)->placeholder('-'),

                Tables\Columns\TextColumn::make('nombre_engagements')
                    ->label('Nb Eng.')->alignCenter()->badge()->color('success'),

                Tables\Columns\TextColumn::make('montant_total')
                    ->label('Montant Total')->money('XAF')->sortable()->weight('bold')->color('success'),

                Tables\Columns\TextColumn::make('detenuPar.name')
                    ->label('Détenu par')->searchable()->badge()->color('warning')->placeholder('-'),

                Tables\Columns\TextColumn::make('jours_attente')
                    ->label('Délai')->suffix(' j')->badge()
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('budget_id')
                    ->label('Budget')
                    ->relationship('budget', 'libelle')->searchable()->preload(),
                ...
                Tables\Filters\SelectFilter::make('detenu_par_id')
                    ->label('Détenu par')
                    ->relationship('detenuPar', 'name')->multiple(),
            ...
            //    Fusion des deux anciens blocs ->actions() en un seul
            // ════════════════════════════════════════════════════════
            ->actions([
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\ViewAction::make()
                        ->label('Voir le détail')->icon('heroicon-o-eye')->color('info'),

                    Tables\Actions\EditAction::make()
                        ->label('Modifier')->icon('heroicon-o-pencil')->color('warning')
                    ...
                    Tables\Actions\Action::make('transmettre')
                        ...
                        ->action(function ($record, array $data) {
                            ...
                            if (!$destinataire->hasAnyRole(['controleur_financier', 'daaf', 'directeur_general'])) {
                                throw new \Exception(
                            ...
                            Notification::make()->title('Bordereau transmis')->success()
                                ->body("Transmis à {$destinataire->name}")->send();
                        }),

                    Tables\Actions\Action::make('receptionner')
                        ->label('Réceptionner')
                        ->icon('heroicon-o-inbox-arrow-down')->color('warning')
                        ->visible(fn($record) => $record->statut === 'transmis')
                        ->requiresConfirmation()
                        ->action(function ($record) {
                            try {
                                $record->receptionner(auth()->user());
                                Notification::make()->title('Bordereau réceptionné')->success()->send();
                            } catch (\Exception $e) {
                    ...
                    Tables\Actions\Action::make('telecharger_pdf')
                        ...
                        ->icon('heroicon-o-document-arrow-down')->color('primary')
                    ...
                    Tables\Actions\Action::make('afficher_pdf')
                        ->label('Aperçu PDF')
                        ->icon('heroicon-o-document-magnifying-glass')->color('gray')
                    ...
                    Tables\Actions\DeleteAction::make()
                        ->label('Supprimer')->icon('heroicon-o-trash')
                        ->visible(fn($record) => $record->statut === 'brouillon')
                        ->requiresConfirmation()

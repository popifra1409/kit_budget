        ...
        return $form
            ->schema([
                    ...
                    ->schema([
                        Forms\Components\Grid::make(3)->schema([
                            Forms\Components\TextInput::make('matricule')
                                ->label('Matricule')->required()->unique(ignoreRecord: true)
                                ->maxLength(50)->placeholder('Ex: MAT-2026-0001')
                                ->helperText('Matricule unique du personnel'),

                            Forms\Components\Select::make('civilite')
                                ->label('Civilité')
                                ->options(['M.' => 'M.', 'Mme' => 'Mme', 'Mlle' => 'Mlle']),

                            Forms\Components\Select::make('sexe')
                                ->label('Sexe')
                                ->options(['M' => 'Masculin', 'F' => 'Féminin'])->required(),
                        ]),

                        Forms\Components\Grid::make(2)->schema([
                            Forms\Components\TextInput::make('nom')->label('Nom')->required()->maxLength(255),
                            Forms\Components\TextInput::make('prenoms')->label('Prénoms')->required()->maxLength(255),
                        ]),

                        Forms\Components\Grid::make(3)->schema([
                            Forms\Components\DatePicker::make('date_naissance')
                                ->label('Date de naissance')->maxDate(now()->subYears(18))->displayFormat('d/m/Y'),
                            Forms\Components\TextInput::make('lieu_naissance')->label('Lieu de naissance')->maxLength(255),
                            Forms\Components\TextInput::make('nationalite')->label('Nationalité')->default('Camerounaise')->maxLength(255),
                        ]),
                    ])
                    ->columns(1),
                    ...
                    ->schema([
                        Forms\Components\Grid::make(2)->schema([
                            ...
                            Forms\Components\TextInput::make('fonction')
                                ->label('Fonction')->required()->maxLength(255)->placeholder('Ex: Chef de Service'),
                        ]),

                        Forms\Components\Grid::make(3)->schema([
                            Forms\Components\TextInput::make('grade')->label('Grade')->maxLength(255),
                            Forms\Components\Select::make('categorie')
                                ->label('Catégorie')
                                ->options(['A' => 'Catégorie A', 'B' => 'Catégorie B', 'C' => 'Catégorie C', 'D' => 'Catégorie D'])
                                ->searchable(),
                            Forms\Components\TextInput::make('echelon')->label('Échelon')->maxLength(255),
                        ]),

                        Forms\Components\TextInput::make('indice')->label('Indice')->maxLength(255),
                    ])
                    ->columns(1),
                    ...
                    ->schema([
                        Forms\Components\Grid::make(3)->schema([
                            Forms\Components\DatePicker::make('date_prise_service')
                                ->label('Date de prise de service')->displayFormat('d/m/Y'),
                            Forms\Components\DatePicker::make('date_titularisation')
                                ->label('Date de titularisation')->displayFormat('d/m/Y')->after('date_prise_service'),
                            Forms\Components\DatePicker::make('date_depart_retraite')
                                ->label('Date de départ à la retraite')->displayFormat('d/m/Y')->after('date_titularisation'),
                        ]),
                    ])
                ...
                Forms\Components\Section::make('Contact')
                    ->schema([
                        Forms\Components\Grid::make(2)->schema([
                            Forms\Components\TextInput::make('telephone')->label('Téléphone')->tel()->maxLength(255)->placeholder('+237 6XX XX XX XX'),
                            Forms\Components\TextInput::make('email')->label('Email')->email()->maxLength(255),
                        ]),
                        Forms\Components\Textarea::make('adresse')->label('Adresse')->rows(2)->columnSpanFull(),
                    ])
                ...
                Forms\Components\Section::make('Informations bancaires et administratives')
                    ->schema([
                        Forms\Components\Grid::make(2)->schema([
                            Forms\Components\TextInput::make('numero_cni')->label('N° CNI')->maxLength(255),
                            Forms\Components\TextInput::make('numero_cnps')->label('N° CNPS')->maxLength(255),
                        ]),
                        Forms\Components\Grid::make(2)->schema([
                            Forms\Components\TextInput::make('numero_compte_bancaire')->label('N° Compte bancaire')->maxLength(255),
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('matricule')
                    ->label('Matricule')->searchable()->sortable()->weight('bold')->copyable(),

                Tables\Columns\TextColumn::make('nom')
                    ->label('Nom')->searchable()->sortable(),

                Tables\Columns\TextColumn::make('prenoms')
                    ->label('Prénoms')->searchable()->sortable(),

                Tables\Columns\TextColumn::make('fonction')
                    ->label('Fonction')->searchable()->wrap(),

                Tables\Columns\TextColumn::make('service.nom')
                    ->label('Service')->searchable()->sortable()->toggleable(),
                ...
                Tables\Columns\TextColumn::make('telephone')
                    ->label('Téléphone')->searchable()->toggleable(),
                ...
                Tables\Columns\IconColumn::make('actif')
                    ->label('Actif')->boolean()
                    ->trueColor('success')->falseColor('danger')->toggleable(),
            ])
            ->filters([
                Tables\Filters\SelectFilter::make('service_id')
                    ->label('Service')->relationship('service', 'nom')->searchable()->preload(),

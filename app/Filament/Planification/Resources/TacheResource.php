...
use App\Filament\Planification\Resources\TacheResource\Pages;
...
class TacheResource extends Resource
{
    ...
    public static function canEdit($record): bool
    {
        ...
        $user = auth()->user();
        if (!$user->can('update_tache')) return false;
                ...
                Forms\Components\Section::make('Hiérarchie')
                    ...
                    ->schema([
                        Forms\Components\Select::make('niveau')
                            ->label('Niveau')
                            ->options(['tache' => 'Tâche principale', 'sous_tache' => 'Sous-tâche'])
                            ->required()->default('tache')->live()
                            ->afterStateUpdated(function ($state, callable $set) {
                                if ($state === 'tache') $set('parent_id', null);
                            })
                ...
                Forms\Components\Section::make('Informations de la Tâche')
                    ->schema([
                        Forms\Components\TextInput::make('code')
                            ->label('Code')->required()->maxLength(50)->placeholder('Ex: T1 ou T1-01'),
                ...
                Forms\Components\Section::make('Gestion et Responsabilité')
                    ->schema([
                        Forms\Components\TextInput::make('delai')
                            ->label('Délai de réalisation')->maxLength(255)
                            ->placeholder('Ex: 3 mois, T1 2026'),
                        Forms\Components\TextInput::make('guichet')
                            ->label('Guichet')->maxLength(255)->placeholder('Ex: Guichet Pharmacie'),
                        Forms\Components\Select::make('service_id')
                            ->label('Service responsable')
                            ->relationship('service', 'nom')->searchable()->preload()
                            ->createOptionForm([
                                Forms\Components\TextInput::make('code')->label('Code')->required()->unique('services', 'code')->maxLength(50),
                                Forms\Components\TextInput::make('nom')->label('Nom')->required()->maxLength(255),
                                Forms\Components\TextInput::make('responsable')->label('Responsable')->maxLength(255),
                            ])
                ...
                Forms\Components\Section::make('Budget')
                    ...
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('ae')
                            ...
                            ->required(fn(callable $get) => $get('niveau') === 'sous_tache')
                            ->disabled(fn(callable $get) => $get('niveau') === 'tache')
                            ->dehydrated(fn(callable $get) => $get('niveau') === 'sous_tache')
                            ->numeric()->default(0)->prefix('FCFA')
                            ->placeholder(fn(callable $get) => $get('niveau') === 'tache' ? 'Calculé automatiquement' : '0')
                            ->helperText(fn(callable $get) => $get('niveau') === 'tache' ? 'Somme des AE des sous-tâches' : 'Montant de l\'autorisation d\'engagement')
                            ->afterStateHydrated(function ($component, $state, $record) {
                                if ($record && $record->estTachePrincipale()) $component->state($record->getTotalAe());
                            }),
                        ...
                        Forms\Components\TextInput::make('cp')
                            ...
                            ->numeric()->default(0)->prefix('FCFA')
                            ->placeholder(fn(callable $get) => $get('niveau') === 'tache' ? 'Calculé automatiquement' : '0')
                            ->helperText(fn(callable $get) => $get('niveau') === 'tache' ? 'Somme des CP des sous-tâches' : 'Montant du crédit de paiement')
                            ->afterStateHydrated(function ($component, $state, $record) {
                                if ($record && $record->estTachePrincipale()) $component->state($record->getTotalCp());
                            }),
                    ])
                    ->columns(2),
                        ...
                        Forms\Components\Placeholder::make('resume_budget')
                            ...
                            ->content(function ($record) {
                                if (!$record || !$record->sousTaches->count()) {
                                    return new \Illuminate\Support\HtmlString('<div class="text-center text-gray-500 dark:text-gray-400 py-4">Aucune sous-tâche. Ajoutez des sous-tâches pour voir les totaux.</div>');
                                }
                                ...
                                return new \Illuminate\Support\HtmlString("
                                    <div class='grid grid-cols-1 md:grid-cols-3 gap-4'>
                                        <div class='rounded-lg border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 p-4 text-center'>
                                            <div class='text-sm font-medium text-blue-600 dark:text-blue-400 mb-2'>📋 Sous-tâches</div>
                                            <div class='text-3xl font-bold text-blue-700 dark:text-blue-300'>{$count}</div>
                                        </div>
                                        <div class='rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-4 text-center'>
                                            <div class='text-sm font-medium text-green-600 dark:text-green-400 mb-2'>💵 Total AE</div>
                                            <div class='text-2xl font-bold text-green-700 dark:text-green-300'>{$ae} <span class='text-sm'>FCFA</span></div>
                                        </div>
                                        <div class='rounded-lg border border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/20 p-4 text-center'>
                                            <div class='text-sm font-medium text-orange-600 dark:text-orange-400 mb-2'>💰 Total CP</div>
                                            <div class='text-2xl font-bold text-orange-700 dark:text-orange-300'>{$cp} <span class='text-sm'>FCFA</span></div>
                                        </div>
                                    </div>
                                    <div class='mt-4 text-xs text-center text-gray-600 dark:text-gray-400'>💡 Ces montants sont calculés automatiquement à partir des {$count} sous-tâche(s)</div>
                                ");
                            }),
                    ])
                    ->visible(fn(callable $get, $record) => $get('niveau') === 'tache' && $record && $record->sousTaches->count() > 0)
                    ->collapsible()->collapsed(false),

                Forms\Components\Section::make('Résultats et Indicateurs')
                    ->schema([
                        Forms\Components\Textarea::make('resultat_attendu')
                            ->label('Résultat attendu')->rows(2)
                            ->placeholder('Ex: Disponibilité permanente des médicaments essentiels'),
                        Forms\Components\Textarea::make('indicateur_resultat')
                            ->label('Indicateur de résultat')->rows(2)
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\BadgeColumn::make('niveau')
                    ->label('Niveau')
                    ->colors(['primary' => 'tache', 'success' => 'sous_tache'])
                ...
                Tables\Columns\TextColumn::make('service.nom')
                    ->label('Service')->searchable()->sortable()->placeholder('-')->toggleable(),

                Tables\Columns\TextColumn::make('ae')
                    ->label('AE')
                    ->formatStateUsing(fn($record) => number_format($record->getTotalAe(), 0, ',', ' ') . ' FCFA')
                    ->sortable()
                    ->summarize([
                        Tables\Columns\Summarizers\Summarizer::make()->label('Total AE')
                            ->using(function ($query) {
                                $tacheIds = (clone $query)->where('niveau', 'tache')->pluck('id');
                                $taches   = \App\Models\Tache::whereIn('id', $tacheIds)->get();
                                return number_format($taches->sum(fn($t) => $t->getTotalAe()), 0, ',', ' ') . ' FCFA';
                            }),
                    ])
                    ->tooltip(
                        fn($record) => $record->estTachePrincipale()
                ...
                Tables\Columns\TextColumn::make('cp')
                    ->label('CP')
                    ->formatStateUsing(fn($record) => number_format($record->getTotalCp(), 0, ',', ' ') . ' FCFA')
                    ->sortable()
                    ->summarize([
                        Tables\Columns\Summarizers\Summarizer::make()->label('Total CP')
                            ->using(function ($query) {
                                $tacheIds = (clone $query)->where('niveau', 'tache')->pluck('id');
                                $taches   = \App\Models\Tache::whereIn('id', $tacheIds)->get();
                                return number_format($taches->sum(fn($t) => $t->getTotalCp()), 0, ',', ' ') . ' FCFA';
                            }),
                    ])
                    ->tooltip(
                        fn($record) => $record->estTachePrincipale()
                ...
                Tables\Columns\TextColumn::make('sousTaches_count')
                    ->label('Sous-tâches')->counts('sousTaches')->badge()->color('success')
                    ->visible(fn($record) => $record && $record->niveau === 'tache')
                    ->toggleable(),
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('parent_id')
                    ->label('Tâche parent')->relationship('parent', 'libelle')
                    ->searchable()->preload()
                ...
                Tables\Filters\SelectFilter::make('activite_id')
                    ->label('Activité')->relationship('activite', 'libelle')->searchable()->preload(),

                Tables\Filters\SelectFilter::make('nomenclature_id')
                    ->label('Nomenclature')->relationship('nomenclature', 'libelle')->searchable()->preload(),
            ...
            ->actions([
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\Action::make('voir_sous_taches')
                        ...
                        ->tooltip('Filtrer les sous-tâches de cette tâche'),

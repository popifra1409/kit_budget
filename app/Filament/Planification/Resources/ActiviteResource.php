...
use App\Filament\Planification\Resources\ActiviteResource\Pages;
...
class ActiviteResource extends Resource
{
    ...
    {
        $user = auth()->user();
        if (!$user?->can('update_activite')) return false;
    ...
    {
        $user = auth()->user();
        if (!$user?->can('delete_activite')) return false;
                ...
                Forms\Components\Section::make('Informations de l\'Activité')
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('code')
                            ->label('Code')->required()->maxLength(50)->placeholder('Ex: ACT1'),
    ...
    public static function getEloquentQuery(): \Illuminate\Database\Eloquent\Builder
    {
        return parent::getEloquentQuery()
            ->with(['exercice', 'action.programme', 'taches.sousTaches']);
    }
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('action.code')
                    ->label('Action')->searchable()->sortable()->badge()->color('info'),
                ...
                Tables\Columns\TextColumn::make('budget_ae')
                    ->label('Budget (AE)')
                    ->state(fn($record) => $record->taches->where('niveau', 'tache')
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('action_id')
                    ->label('Action')
                    ->relationship('action', 'libelle')->searchable()->preload(),
